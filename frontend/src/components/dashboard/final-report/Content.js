...
export const addVariables = (
  ...
  string = string?.replace(
    "**CLAIMSERVICINGOFFICE**",
    `<strong>${claimServicingOffice}</strong>`
  );
  string = string?.replace(
    "**ALLOTMENTDATE**",
    `<strong>${formatDate(AllotmentDate)}</strong>`
  );
  string = string?.replace(
    "**DATEOFACCIDENT**",
    `<strong>${formatDate(DateOfAccident)}</strong>`
  );
  string = string?.replace(
    "**PLACEOFACCIDENT**",
    `<strong>${PlaceOfAccident}</strong>`
  );
  string = string?.replace("**INSUREDNAME**", `<strong>${DriverName}</strong>`);
  string = string?.replace(
    "**CASSISNUMBER**",
    `<strong>${ChassisNumber}</strong>`
  );
  string = string?.replace(
    "**POLICYNUMBER**",
  ...
  string = string?.replace(
    "**TIMEOFACCIDENT**",
    `<strong>${convertToReadable(TimeOfAccident)}</strong>`
  );
