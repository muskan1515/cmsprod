 ...
 const sendEmail1 = (req, res) => {
    ...
    db.query(sql, [leadId], (err, result) => {
      ...
      ${content}
        ...
        Please provide the clear copy of all the documents so that the claim processing can be fast or
        <p><a href=https://claims-backend-apis.onrender.com/documents/${leadId}?token=${sendingToken}&type=${type}&content=${"Videos"} target="_blank">Click me</a> to fill the documents information .</p>
  
        Please provide the clear copy of all the documents so that the claim processing can be fast or
        <p><a href=https://claims-backend-apis.onrender.com/documents/${leadId}?token=${sendingToken}&type=${type}&content=${"Images"} target="_blank">Click me</a> to fill the documents information .</p>
